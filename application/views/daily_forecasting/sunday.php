...
<div style="margin: 0 auto; width: 90%;margin-top: 10px;">
	...
	<li class="active"><a href="<?php echo base_url('daily_forecasting/sunday?startdate='.$startdate.'&end_date='.$end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type); ?>">Sunday</a></li>
	...
	<div style="width:99%;margin: 0 auto;">
		...
		<span style="float: left;font-size:14px;"><b><?php echo $this->session->userdata('branch'); ?></b> | <b>
				...
				<?php else: ?>
				  Taco Bell
				<?php endif; ?>    
			</b> | <b><?php echo $sixthday_display; ?></b>
		 </span>
		</div>
		<br/>
		...
		<form action="<?php echo base_url('daily_forecasting/adjfo6?startdate=') . $startdate . '&end_date=' . $end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type; ?>" method="POST" id="df_sunday">
			
			<div style="border-radius: 5px; box-shadow: 0px 0px 2px 1px #888888; margin-top: 60px;">

				<input type="hidden" name="daysix_date" id="daysix_date" value="<?php echo $f_sixthdate_display; ?>" />	

				<table class="table table-hover" class="tbl_sunday">
                ...
                <thead>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;border-left:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;font-family:Tahoma;">
							<?php 
								$split_fdate_s = explode("-",$sixthdate_display);
						...
						<td style="border-right:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;">
							<?php 
								 ...
								 $date_sun = date("Y-m-d", strtotime($startdate . " +5 day"));	
								 $split_datef_6 = explode("-",$date_sun);
								echo $split_datef_6[1] . '/' . $split_datef_6[2] . '/' . $split_datef_6[0];
							 ?>
						</b></td>
						<td></td>
						<td></td>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;font-size:12px;text-align:center;font-weight:bold;color:#666;"><input type="text" id="forecast_amount_design" class="daysix_forecasted_amount" value="<?php echo $forecasted_amount6->forecast_amount; ?>" placeholder="Type here."></td>
...
$('.daysix_forecasted_amount').each(function(){
$(this).change(function(){

if(!$.isNumeric($(this).val())) {
   alert('Only Numeric value was allow.');
   $('#df_sunday').find('.'+'daysix_forecasted_amount').val('');
   return true;
...
$.ajax({
type: "POST",
url:  domain+'/daily_forecasting/update6',
data:  {forecasted_amount: $(this).val(), daysix_date: $('#daysix_date').val(), daysix_ref_date: $('#ref_date').val()},
dataType: "json",
...
if($('.daysix_forecasted_amount').val()==""){
	alert("Fill-up Forecast sales first.");
	$('#df_sunday').find('#'+id).val('');
   	$('#df_sunday').find('.'+id).val('');
   	return true;
...
if(!$.isNumeric($(this).val())) {
   ...
   return true;
	...
	if(value==""){
   		$('#df_sunday').find('.'+id).val(id+'/'+$(this).val());
