...
<div style="margin: 0 auto; width: 90%;margin-top: 10px;">
	...
	<li class="active"><a href="<?php echo base_url('daily_forecasting/tuesday?startdate='.$startdate.'&end_date='.$end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type); ?>">Tuesday</a></li>
	...
	<div style="width:99%;margin: 0 auto;">
		...
		<span style="float: left;font-size:14px;"><b><?php echo $this->session->userdata('branch'); ?></b> | <b>
				...
				<?php else: ?>
				  Taco Bell
				<?php endif; ?>    
			</b> | <b><?php echo $firstday_display; ?></b>
		</span>	
		</div>
		<br/>
		...
		<form action="<?php echo base_url('daily_forecasting/adjfo1?startdate=') . $startdate . '&end_date=' . $end_date . '&ref_startdate=' . $ref_startdate . '&ref_end_date=' . $ref_end_date . '&type=' . $type; ?>" method="POST" id="df_tuesday">
			<div style="border-radius: 5px; box-shadow: 0px 0px 2px 1px #888888; margin-top: 60px;">

				<input type="hidden" name="dayone_date" id="dayone_date" value="<?php echo $f_firstdate_display; ?>" />	
                ...
                <thead>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;border-left:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;font-family:Tahoma;">
							<?php 
								$split_fdate_s = explode("-",$firstdate_display);
						...
						<td style="border-right:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;">
							<?php 
								 $split_fdate = explode("-",$datef_start);
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;font-size:12px;text-align:center;font-weight:bold;color:#666;"><input type="text" id="forecast_amount_design" class="dayone_forecasted_amount" value="<?php echo $forecasted_amount->forecast_amount; ?>" placeholder="Type here."></td>
...
if(!$.isNumeric($(this).val())) {
   ...
   $('#df_tuesday').find('.'+'dayone_forecasted_amount').val('');
   return true;
...
data:  {forecasted_amount: $(this).val(), dayone_date: $('#dayone_date').val(), dayone_ref_date: $('#ref_date').val()},
dataType: "json",
...
if($('.dayone_forecasted_amount').val()==""){
	alert("Fill-up Forecast sales first.");
	$('#df_tuesday').find('#'+id).val('');
   	$('#df_tuesday').find('.'+id).val('');
   	return true;
...
if(!$.isNumeric($(this).val())) {
   ...
   return true;
	...
	if(value==""){
   		$('#df_tuesday').find('.'+id).val(id+'/'+$(this).val());
