...
<div style="margin: 0 auto; width: 90%;margin-top: 10px;">
	...
	<li class="active"><a href="<?php echo base_url('daily_forecasting/friday?startdate='.$startdate.'&end_date='.$end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type); ?>">Friday</a></li>
	...
	<li><a href="<?php echo base_url('daily_forecasting/monday?startdate='.$startdate.'&end_date='.$end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $types); ?>">Monday</a></li>
	...
	<div style="width:99%;margin: 0 auto;">
		...
		<span style="float: left;font-size:14px;"><b><?php echo $this->session->userdata('branch'); ?></b> | <b>
				...
				<?php else: ?>
				  Taco Bell
				<?php endif; ?>    
			</b> | <b><?php echo $fourthday_display; ?></b>
		  </span>
		</div>
		<br/>
		...
		<form action="<?php echo base_url('daily_forecasting/adjfo4?startdate=') . $startdate . '&end_date=' . $end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type; ?>" method="POST" id="df_friday">	
			
			<div style="border-radius: 5px; box-shadow: 0px 0px 2px 1px #888888; margin-top: 60px;">

				<input type="hidden" name="dayfour_date" id="dayfour_date" value="<?php echo $f_fourthdate_display; ?>" />	

				<table class="table table-hover" class="tbl_friday">
                ...
                <thead>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;border-left:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;font-family:Tahoma;">
							<?php 
								$split_fdate_s = explode("-",$fourthdate_display);
						...
						<td style="border-right:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;">
							<?php 
								 ...
								 $date_fri = date("Y-m-d", strtotime($startdate . " +3 day"));	
								 $split_datef_4 = explode("-",$date_fri);
								echo $split_datef_4[1] . '/' . $split_datef_4[2] . '/' . $split_datef_4[0];
							 ?>
						</b></td>
						<td></td>
						<td></td>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;font-size:12px;text-align:center;font-weight:bold;color:#666;"><input type="text" id="forecast_amount_design" class="dayfour_forecasted_amount" value="<?php echo $forecasted_amount4->forecast_amount; ?>" placeholder="Type here."></td>
...
$('.dayfour_forecasted_amount').each(function(){
$(this).change(function(){

if(!$.isNumeric($(this).val())) {
   alert('Only Numeric value was allow.');
   $('#df_friday').find('.'+'dayfour_forecasted_amount').val('');
   return true;
...
$.ajax({
type: "POST",
url:  domain+'/daily_forecasting/update4',
data:  {forecasted_amount: $(this).val(), dayfour_date: $('#dayfour_date').val(), dayfour_ref_date: $('#ref_date').val()},
dataType: "json",
...
if($('.dayfour_forecasted_amount').val()==""){
	alert("Fill-up Forecast sales first.");
	$('#df_friday').find('#'+id).val('');
   	$('#df_friday').find('.'+id).val('');
   	return true;
...
if(!$.isNumeric($(this).val())) {
   ...
   return true;
	...
	if(value==""){
   		$('#df_friday').find('.'+id).val(id+'/'+$(this).val());
