	...
	<div style="margin: 0 auto; width: 90%;margin-top: 10px;">
		...
		<li class="active"><a href="<?php echo base_url('daily_forecasting/monday_forecast?start='.$startdate.'&end='.$end_date.'&version='.$version); ?>">Monday</a></li>
	...
	<!--<?php if($role_id == 2): ?>
		...
		<br/>
			...
			<form action="<?php echo base_url('daily_forecasting/u_adjfo7?startdate=') . $startdate . '&end_date=' . $end_date . '&version=' . $version; ?>" method="POST" id="df_monday">
				
				<div style="border-radius: 5px; box-shadow: 0px 0px 2px 1px #888888; margin-top: 60px;">

				<input type="hidden" name="dayseven_date" id="dayseven_date" value="<?php echo $seventhdate_display; ?>" />	
                ...
                <thead>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;">
							<?php 
								$split_fdate = explode("-",$seventhdate_display);
   ...
   return true;
	...
	$.ajax({
	type: "POST",
	url:  domain+'/daily_forecasting/u_update7',
	data:  {forecasted_amount: $(this).val(), dayseven_date: $('#dayseven_date').val()},
	dataType: "json",
