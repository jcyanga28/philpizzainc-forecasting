...
<div style="margin: 0 auto; width: 90%;margin-top: 10px;">
	...
	<li class="active"><a href="<?php echo base_url('daily_forecasting/saturday?startdate='.$startdate.'&end_date='.$end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type); ?>">Saturday</a></li>
	...
	<div style="width:99%;margin: 0 auto;">
		...
		<form action="<?php echo base_url('daily_forecasting/adjfo5?startdate=') . $startdate . '&end_date=' . $end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type; ?>" method="POST" id="df_saturday">	
			
			<div style="border-radius: 5px; box-shadow: 0px 0px 2px 1px #888888; margin-top: 60px;">

				<input type="hidden" name="dayfive_date" id="dayfive_date" value="<?php echo $f_fifthdate_display; ?>" />	
                ...
                <thead>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;border-left:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;font-family:Tahoma;">
							<?php 
								$split_fdate_s = explode("-",$fifthdate_display);
						...
						<td style="border-right:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;">
							<?php 
								 ...
								 $date_sat = date("Y-m-d", strtotime($startdate . " +4 day"));	
								 $split_datef_5 = explode("-",$date_sat);
								echo $split_datef_5[1] . '/' . $split_datef_5[2] . '/' . $split_datef_5[0];
							 ?>
						</b></td>
						<td></td>
						<td></td>
...
$.ajax({
type: "POST",
url:  domain+'/daily_forecasting/update5',
data:  {forecasted_amount: $(this).val(), dayfive_date: $('#dayfive_date').val(), dayfive_ref_date: $('#ref_date').val()},
dataType: "json",
