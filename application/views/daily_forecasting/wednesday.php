...
<div style="margin: 0 auto; width: 90%;margin-top: 10px;">
	...
	<li class="active"><a href="<?php echo base_url('daily_forecasting/wednesday?startdate='.$startdate.'&end_date='.$end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type); ?>">Wednesday</a></li>
	...
	<div style="width:99%;margin: 0 auto;">
		...
		<span style="float: left;font-size:14px;"><b><?php echo $this->session->userdata('branch'); ?></b> | <b>
				...
				<?php else: ?>
				  Taco Bell
				<?php endif; ?>    
			</b> | <b><?php echo $secondday_display; ?></b> 
			</span>
		</div>
		<br/>
		...
		<form action="<?php echo base_url('daily_forecasting/adjfo2?startdate=') . $startdate . '&end_date=' . $end_date .'&ref_startdate='.$ref_startdate.'&ref_end_date='.$ref_end_date. '&type=' . $type; ?>" method="POST" id="df_wednesday">	
			
			<div style="border-radius: 5px; box-shadow: 0px 0px 2px 1px #888888; margin-top: 60px;">

				<input type="hidden" name="daytwo_date" id="daytwo_date" value="<?php echo $f_seconddate_display; ?>" />	

				<table class="table table-hover" class="tbl_wednesday">
                ...
                <thead>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;border-left:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;font-family:Tahoma;">
							<?php 
								$split_fdate_s = explode("-",$seconddate_display);
						...
						<td style="border-right:solid 2px #eee;text-align:center;"><b style="font-size: 11px;color:#666;">
							<?php 
								 ...
								 $date_wed = date("Y-m-d", strtotime($startdate . " +1 day"));	
								 $split_datef_2 = explode("-",$date_wed);
								echo $split_datef_2[1] . '/' . $split_datef_2[2] . '/' . $split_datef_2[0];
							 ?>
						</b></td>
						<td></td>
						<td></td>
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;font-size:12px;text-align:center;font-weight:bold;color:#666;"><input type="text" id="forecast_amount_design" class="daytwo_forecasted_amount" value="<?php echo $forecasted_amount2->forecast_amount; ?>" placeholder="Type here."></td>
...
$('.daytwo_forecasted_amount').each(function(){
$(this).change(function(){

if(!$.isNumeric($(this).val())) {
   alert('Only Numeric value was allow.');
   $('#df_wednesday').find('.'+'daytwo_forecasted_amount').val('');
   return true;
...
$.ajax({
type: "POST",
url:  domain+'/daily_forecasting/update2',
data:  {forecasted_amount: $(this).val(), daytwo_date: $('#daytwo_date').val(), daytwo_ref_date: $('#ref_date').val()},
dataType: "json",
...
if($('.daytwo_forecasted_amount').val()==""){
	alert("Fill-up Forecast sales first.");
	$('#df_wednesday').find('#'+id).val('');
   	$('#df_wednesday').find('.'+id).val('');
   	return true;
...
if(!$.isNumeric($(this).val())) {
   ...
   return true;
	...
	if(value==""){
   		$('#df_wednesday').find('.'+id).val(id+'/'+$(this).val());
