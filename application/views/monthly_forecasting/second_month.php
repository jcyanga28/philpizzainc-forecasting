...
<div style="margin: 0 auto; width: 90%;margin-top: 10px;">
	...
	<li class="active"><a href="<?php echo base_url('monthly_forecasting/second_month?monthstart='.$monthstart.'&monthend='.$monthend.'&ref_monthstart='.$ref_monthstart.'&ref_month_end='.$ref_month_end.'&type='.$type); ?>">2nd Month</a></li>
	...
	<div style="width:90%;margin: 0 auto;">
		...
		<form action="<?php echo base_url('monthly_forecasting/adjfo2?monthstart=') . $monthstart . '&monthend=' . $monthend.'&ref_monthstart='.$ref_monthstart.'&ref_month_end='.$ref_month_end.'&type='.$type; ?>" method="POST" id="mf_smonth">
			
			<div style="border-radius: 5px; box-shadow: 0px 0px 2px 1px #888888; margin-top: 60px;">

				<input type="hidden" name="smonth_from" id="smonth_from" value="<?php echo $ssmonth_from; ?>" />
				<input type="hidden" name="smonth_end" id="smonth_end" value="<?php echo $ssmonth_end; ?>" />	
				
				<table class="table table-hover" class="tbl_smonth">
				...
				<tr style="font-size:11px;">
					...
					<tr>
						...
						<td style="border-right:solid 2px #eee;font-size:12px;text-align:center;font-weight:bold;color:#666;"><input type="text" id="forecast_amount_design" class="smonth_forecasted_amount" value="<?php echo $forecasted_amount->forecast_amount; ?>" placeholder="Type here."></td>
...
$('.smonth_forecasted_amount').each(function(){
$(this).change(function(){

if(!$.isNumeric($(this).val())) {
   alert('Only Numeric value was allow.');
   $('#mf_smonth').find('.'+'smonth_forecasted_amount').val('');
   return true;
...
$.ajax({
type: "POST",
url:  domain+'/monthly_forecasting/update2',
data:  {forecasted_amount: $(this).val(), date_from: $('#smonth_from').val(), date_to: $('#smonth_end').val(), ref_date_from: $('#ref_month_start').val(), ref_date_to: $('#ref_month_to').val()},
dataType: "json",
...
if($('.smonth_forecasted_amount').val()==""){
	alert("Fill-up Forecast sale first.");
	$('#mf_smonth').find('#'+id).val('');
   	$('#mf_smonth').find('.'+id).val('');
   	return true;
...
if(!$.isNumeric($(this).val())) {
   ...
   return true;
	...
	if(value==""){
   		$('#mf_smonth').find('.'+id).val(id+'/'+$(this).val());
